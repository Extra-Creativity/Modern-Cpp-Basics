...
class ITCPStream : public std::basic_istream<char>
{
    using Base = std::basic_istream<char>;

public:
    ITCPStream() : Base{ &buf_ } {}

    void open(Socket &&socket, std::streamsize inSize)
    {
        buf_.open(std::move(socket), std::ios::in, inSize, 0);
    }
